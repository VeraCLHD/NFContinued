...
public class QuestionsDoctorLinksCollector extends LinksCollector {

	private static QuestionsDoctorLinksCollector instance = null;
	private final String QUESTION_STARTPAGE = "http://nutritionfacts.org/questions/";
	
	private QuestionsDoctorLinksCollector(){ }
	
	public static QuestionsDoctorLinksCollector getInstance(){
		 if(instance == null) {
			 instance = new QuestionsDoctorLinksCollector();
			 instance.setOutput_path(Properties.QUESTION_DOCTOR_LINKS_PATH);
		     }
		 return instance;
	   } 
	
	/**
	 * Starting at the ask the doctor questions main page, collects all links to questions and writes them in a file.
	 */
	@Override
	public void crawlQueryLinks() {
		...
		// both 'ask the doctor' and 'ask the dietrician' pages
			assert(Properties.QUESTIONS_DOCTOR_PAGE_PATTERN.matcher(QUESTION_STARTPAGE).matches());
			...
			while (true) {
				try {
					...
					for (Element link_element : links) {
						...
						if (QueryPageCrawler.isQuestionDoctorLink(link) == true && questionlinks_set.contains(link) == false) {
							...
							io.Writer.appendLineToFile(link, Properties.QUESTION_DOCTOR_LINKS_PATH);
	...
	/**
	 * Updates older link files with only the chronologically newest doctor question links.
	 */
	public void updateLinks() {
		...
		// both 'ask the doctor' and 'ask the dietrician' pages
			...
			io.Editor.deleteFile(Properties.QUESTION_DOCTOR_LINKS_PATH);
